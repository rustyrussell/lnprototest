...
def test_anchor_commitment() -> None:
    ...
    # ```
    yamlstr = ''
    for line in open("lnprototest/commit_tx.py", 'r').readlines():
        if not line.startswith('    # '):
            continue
        line = line[len('    # '):]
        if yamlstr == '' and line == "```yaml\n":
            yamlstr = '\n'
        elif yamlstr != '':
            if line == "```\n":
                break
    ...
    tx, _ = c._unsigned_tx(Side.local)
    # We don't (yet) generate witnesses, so compare txids.
    assert tx.GetTxid() == CMutableTransaction.deserialize(bytes.fromhex(tests[0]['ExpectedCommitmentTxHex'])).GetTxid()
